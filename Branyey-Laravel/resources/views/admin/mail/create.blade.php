                ...
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-body p-4">
                        ...
                        <div class="form-check mode-pill mb-2">
                            <input class="form-check-input" type="radio" name="recipient_mode" id="mode_all" value="all" {{ old('recipient_mode', 'all') === 'all' ? 'checked' : '' }}>
                            <label class="form-check-label" for="mode_all">Todos los usuarios activos con correo</label>
                        </div>

                        <div class="form-check mode-pill mb-2">
                            <input class="form-check-input" type="radio" name="recipient_mode" id="mode_role" value="role" {{ old('recipient_mode') === 'role' ? 'checked' : '' }}>
                        ...
                        <div class="form-check mode-pill mb-2">
                            <input class="form-check-input" type="radio" name="recipient_mode" id="mode_selected" value="selected" {{ old('recipient_mode') === 'selected' ? 'checked' : '' }}>
                            ...
                            <div class="user-list border rounded-3 p-2">
                                @foreach($users as $user)
                                    <div class="form-check user-item p-2 rounded-3"
                                         data-search="{{ strtolower(($user->nombre_completo ?? '') . ' ' . ($user->email ?? '') . ' ' . ($user->rol->nombre ?? '')) }}">
                                        <input class="form-check-input user-checkbox" type="checkbox" name="selected_users[]" id="user_{{ $user->id }}" value="{{ $user->id }}" {{ in_array($user->id, old('selected_users', [])) ? 'checked' : '' }}>
                                        <label class="form-check-label ms-1" for="user_{{ $user->id }}">
                                            <span class="fw-semibold">{{ $user->nombre_completo ?: $user->email }}</span><br>
                                            <small class="text-muted">{{ $user->email }}</small>
                                            <span class="badge text-bg-light border ms-1">{{ ucfirst($user->rol->nombre ?? 'sin rol') }}</span>
                                        </label>
                                    </div>
                                @endforeach
                            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="subject" name="subject" value="{{ old('subject') }}" placeholder="Ej: Nuevo catálogo disponible esta semana" required>
                        </div>

                        <div class="mb-3">
                            <label for="message" class="form-label">Mensaje</label>
                            <textarea class="form-control" id="message" name="message" rows="11" required>{{ old('message') }}</textarea>
                        </div>
