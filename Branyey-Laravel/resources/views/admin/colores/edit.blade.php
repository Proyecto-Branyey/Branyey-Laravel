    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="fw-bold mb-1 text-dark">
                <i class="bi bi-pencil-square me-2 text-secondary"></i>Editar Color
            </h1>
            <p class="text-muted small mb-0">Modifica la información del color</p>
        </div>
        ...
        <div class="col-lg-6">
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('admin.colores.update', $colore) }}" method="POST" id="colorForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nombre" id="nombre" class="form-control rounded-3" 
                                   value="{{ old('nombre', $colore->nombre) }}" required>
                            ...
                            <div class="row g-3 align-items-end">
                                <div class="col-8">
                                    <input type="text" name="codigo_hex" id="codigo_hex" class="form-control rounded-3" 
                                           value="{{ old('codigo_hex', $colore->codigo_hex ?? '#000000') }}" maxlength="7">
                                    <div class="form-text text-muted small mt-1">Formato hexadecimal (ej: #FF5733)</div>
                                </div>
                                <div class="col-4">
                                    <div class="color-preview-wrapper">
                                        <div id="colorPreview" class="color-preview" style="background-color: {{ $colore->codigo_hex ?? '#000000' }};"></div>
                                        <input type="color" id="colorPicker" class="color-picker" value="{{ $colore->codigo_hex ?? '#000000' }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-2 pt-3 border-top">
                            <button type="submit" class="btn btn-dark px-4">
                                <i class="bi bi-save me-2"></i> Actualizar color
                            </button>
