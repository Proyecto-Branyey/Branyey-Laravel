                ...
                <div class="sticky-top" style="top: 100px;">
                    ...
                    <div class="d-flex gap-2 overflow-auto pb-2" id="gallery-thumbs">
                        @foreach($producto->imagenes as $img)
                            <img src="{{ Storage::url($img->url) }}" class="rounded-4 thumb-img border cursor-pointer" 
                                 style="width: 80px; height: 100px; object-fit: cover;"
                                 data-full="{{ Storage::url($img->url) }}" data-color-id="{{ $img->color_id }}"
            ...
            <div class="col-lg-5">
                <div class="ps-lg-4">
                    <span class="badge bg-dark rounded-pill px-3 py-2 mb-3 text-uppercase tracking-widest" style="font-size: 10px;">
                        {{ $producto->clasificacion->nombre ?? 'Colección' }} • Colección 2026
                    </span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    colorRadios.forEach(radio => {
        radio.addEventListener('click', function() {
            ...
            const thumb = document.querySelector('.thumb-img[data-color-id="'+colorId+'"]');
            if(thumb) document.getElementById('main-product-image').src = thumb.dataset.full;
        });
    });
